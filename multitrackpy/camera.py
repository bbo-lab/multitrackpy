...
def triangulate_points_nocorr(AB, offsets, calib, linedist_thres):
    ...
    full_dirs[maincam, :, :], cambases[maincam, :] = calc_image2space(AB[maincam].T, offsets[maincam],
                                                                      **calib[maincam])
    ...
    for (iC, ab) in enumerate(AB):
        ...
        dirs, cambases[iC] = calc_image2space(ab.T, offsets[iC], **calib[iC])

        distances = np.array([[geometry.get_line_dist(cambases[maincam], full_dirs[maincam, i, :], cambases[iC],
