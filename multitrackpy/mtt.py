...
def read_calib(mtt_path):
    ...
    for ci in calind:
        w2cR = np.squeeze(np.asarray(mtt_file['mt']['mc']['Rglobal'])[ci]) # The way matlab saves arrays alread includes a permute!
        ...
        calibs.append( {'w2cR': w2cR, 'w2cT': w2cT, 'camA': camA, 'camk': camk, 'sensorsize': sensorsize, 'scalepixels': scalepixels } );
...
def read_video_paths(vid_dir,mtt_path):
    mtt_file = h5py.File(mtt_path)
    istracking = np.squeeze(np.asarray([mtt_file['mt']['cam_istracking']])==1)
    return [ vid_dir+''.join([ chr(c) for c in mtt_file[mtt_file['mt']['vidname'][0,i]][:].T.astype(np.int)[0] ]) for i in np.where(istracking)[0]]
